...
router.get('/transfers/pending', requirePicker, (req, res, next) => {
  try {
    const pending = getPendingTransferForUser(req.user.id, ROLES.PICKER);
    const enriched = enrichTransferWithUsers(pending);
    res.json({ data: enriched, total: enriched.length });
  } catch (e) { next(e); }
});

router.get('/transfers/my', requirePicker, (req, res, next) => {
  try {
    ...
    const result = getMyTransferList(req.user.id, ROLES.PICKER, filters, page, pageSize);
...
router.post('/transfers/:id/accept', requirePicker, (req, res, next) => {
  try {
...
router.post('/transfers/:id/reject', requirePicker, validateBody({
...
router.get('/transfers/stats', requirePicker, (req, res, next) => {
  try {
...
router.get('/transfer-reject-reasons', requirePicker, (req, res, next) => {
  try {
